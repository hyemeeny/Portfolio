...
export const playAutoData = [
  {
    title: '이용요금 페이지',
    description: [
      '플랜 탭·요금제·비교표 데이터를 객체 기반으로 구조화하여 사용자가 버전별 요금을 한눈에 확인할 수 있도록 구현했습니다.',
      '결제주기별 할인율과 정기결제 추가 할인, 특가 버전 예외 가격까지 계산 로직으로 처리하고, 플랜별 이용하기 버튼을 현재 버전·특수 버전·일반 버전으로 분기 처리했습니다.',
      '토글 버튼으로 정기결제 할인 적용 여부를 전환하면 요금이 실시간으로 재계산되도록 구현했으며, 툴팁으로 각 요금제의 상세 기능 정보를 제공했습니다.',
      'PC 환경은 드롭다운형 비교표, 모바일 환경은 탭형 비교표로 분리 렌더링하고, 시맨틱 테이블 마크업으로 요금표를 구조화했습니다.',
      '마우스 드래그로 스크롤 가능한 탭 메뉴를 구현하고, 탭 클릭 시 URL 쿼리 파라미터와 동기화되도록 처리했습니다.',
      '하나의 페이지에 플토2.0·GLOBAL·EMP 탭 콘텐츠가 모두 통합되어 있던 레거시 구조를, 페이지 리뉴얼을 계기로 각 탭별 개별 컴포넌트로 분리해 코드 가독성과 유지보수성을 개선했습니다.',
    ],
  ...
  },
  {
    title: '통합솔루션 소개 페이지',
    description: [
      'wheel 이벤트를 감지해 섹션 단위로 이동하는 풀페이지 스크롤 기능을 데스크톱 환경에 구현하고, 상단 배너 유무에 따라 섹션 높이를 동적으로 계산해 반영했습니다.',
      '각 요소에 순차적인 애니메이션 지연 시간을 부여해 섹션 진입 시 콘텐츠가 단계적으로 나타나도록 구현했습니다.',
      '터치 이벤트 기반 스크롤 제어 시 사파리 등 모바일 브라우저에서 링크 클릭과 충돌하는 문제가 발생해, 태블릿·모바일 구간은 JS 대신 CSS Scroll Snap으로 전환해 크로스브라우징 문제를 해결하고 부드러운 섹션 전환을 구현했습니다.',
  ...
  },
  {
    title: '브랜드타겟 페이지',
    description: [
      'IntersectionObserver로 현재 뷰포트에 노출된 섹션을 감지하고, 상태 업데이트를 통해 섹션별로 버튼 텍스트가 동적으로 변경되도록 구현했습니다.',
      '브랜드 로고에 서로 엇갈리는 무한 슬라이드 애니메이션을 CSS keyframe으로 구현하고, will-change 속성을 활용해 끊김 없는 렌더링 성능을 확보했습니다.',
  ...
  },
  {
    title: '셀핏AI 페이지',
    description: [
      '메인 이미지에 마우스 오버 시 transform: scale()과 cubic-bezier 트랜지션을 적용해 부드러운 확대 효과와 텍스트가 함께 노출되도록 구현했습니다.',
      'IntersectionObserver로 각 섹션의 뷰포트 진입 여부를 감지하고, 카드별로 순차적인 지연 시간을 적용한 fade-in/slide-up 애니메이션을 구현했습니다.',
      'position: sticky를 활용해 스크롤 중에도 하단에 고정되는 플로팅 배너를 구현하고, 태블릿·모바일 환경별로 높이와 폰트 크기를 별도 대응했습니다.',
      'react-lazy-load-image-component로 이미지 지연 로딩을 적용해 초기 로딩 성능을 개선했습니다.',
      'next-seo의 DefaultSeo를 활용해 title, description, canonical, Open Graph 메타태그를 적용해 검색 노출 환경을 최적화했습니다.',
    ],
  ...
  },
  {
    title: '프로모션 페이지',
    description: [
      'React state와 이벤트 핸들러를 활용해 당첨 확인 모달의 열기·닫기, 오버레이 클릭 감지, ESC 키 닫기 인터랙션을 구현했습니다.',
      '마케팅·영업팀과의 협업을 통해 다수의 이벤트·프로모션 페이지를 일정에 맞춰 신속하게 퍼블리싱했습니다.',
