...
export const travelmakerPageData = [
  ...
  },
  {
    title: "내 프로필 페이지",
    description: [
      "번들 크기를 줄이고 초기 로딩 속도를 향상시키며 보안을 강화하기 위해, 내 프로필 페이지와 헤더의 유저 데이터 패칭을 서버 컴포넌트로 구현하였습니다.",
      "사용성 테스트 및 피드백 수집을 통해 공용 메뉴바와 사이드바를 제작하여 코드 중복을 줄이고 웹페이지 사용성 개선하였습니다.",
      "Zod와 React-Hook-Form으로 유효성 검사를 추가해 사용자 입력의 정확성과 안정성을 강화하였습니다.",
    ],
  ...
  },
  {
    ...
    description: [
      "성능 최적화와 보안을 강화하기 위해 수정 페이지의 정적 렌더링 데이터 패칭을 서버 컴포넌트로 구현하였습니다. 이를 통해 불필요한 클라이언트 렌더링을 줄이고, 서버에서 직접 데이터를 처리함으로써 보안을 더욱 강화하였습니다.",
      "내 체험 등록 및 수정 페이지를 하나의 컴포넌트로 제작하여 코드의 중복을 줄였습니다.",
      "POST와 PATCH를 Server Action으로 RESTful API를 연동하여 백엔드와의 원활한 데이터 통신을 구현하였습니다.",
      "사용자 편의성을 위해 React-Daum-Postcode를 통한 주소 검색 기능을 추가하였습니다.",
      "UI 개선을 위해 react-datepicker 캘린더를 커스텀 하였습니다.",
    ],
